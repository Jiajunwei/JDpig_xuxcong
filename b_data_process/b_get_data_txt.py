...
if __name__ == '__main__':
    ...
    validation_file = [];
    for i in range(0, 50):
        train = open(data_directory + 'txt\\train_data' + str(i) + '.txt', 'w')
        validation = open(data_directory + 'txt\\validation_data' + str(i) + '.txt', 'w');
        train_file.append(train);
        validation_file.append(validation);

    data_path = os.path.join(data_directory, 'train');
    train_num = 0;
    validation_num = 0;
    all_num = 0;
    for i in range(1, 6):
        nowfile = os.path.join(data_path, str(10+i));
